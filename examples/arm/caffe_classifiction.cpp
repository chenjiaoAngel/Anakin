...
void test_net(const std::string model_file_name, const std::string image_file_name, \
    ...
    for (int i = 0; i < test_iter; i++) {
        ...
        vector<float> test_rst;
        for(int j = 0; j < vout.size(); j++){
            Tensor4hf* tensor_out = vout[j];
           // LOG(INFO) << "extract data: size: " << tensor_out->valid_size() << \
            ...
            double mean_val = 0.0;
            for (int k = 0; k < tensor_out->valid_size(); k++) {
                mean_val += ptr_out[k];
                LOG(INFO) <<"iterations:"<< i <<", k: " << k<< ", output: " <<ptr_out[k];
                /*
                std::string locate = "/home/chenjiao04/Documents/rst.txt";
                FILE* fp = fopen(locate, "w+");
                if (fp == 0) {
                    std::cout << "File open failed: " << locate << std::endl;
                } else {
                    fprintf(fp, "%s, %d, %.18f \n", tensor_out->vout_name, k, score);
                    fclose(fp);
                }
                */
            }
           // LOG(INFO) <<"iterations:"<< i <<", " << j << ", output mean val: " << mean_val / tensor_out->valid_size();
        }
    }
  /*  for(int i = 0; i < test_rst.size;i++){
        LOG(INFO) << "output mean val: " << test_rst[i] / test_iter;
    }
    */
    my_time.end(ctx);
